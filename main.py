...
_DG_KEYWORDS_ALL: list[str] = [
    kw.strip() for kw in os.environ.get(
        "DOMEGGOOK_KEYWORDS",
        (
            # ★ Grok 트렌드 리서치 신규 (2026-07-28 — 우선순위 최상위)
            "빨래바구니,국자홀더,핸디선풍기,실리콘채반,샤워바구니,실리콘보자기,턴테이블,젤리크로스백,냉감베개커버,"
            # 생활/주방
            ...
            # 시계/주얼리 (GATE 포함 — 검색 키워드 신규 추가)
            "시계,손목시계,패션시계,귀걸이,목걸이,반지,팔찌,주얼리,"
            # 이너웨어/속옷 (GATE 포함 — 검색 키워드 신규 추가)
            "이너웨어,속옷세트,브라렛,여성속옷,남성속옷,"
            # 아우터 (GATE 포함 — 검색 키워드 신규 추가)
            "바람막이,경량아우터,봄점퍼,후드집업,"
            # 뷰티 세분화 (쿠팡 동기화 — 바디케어·메이크업·남성뷰티)
            "바디버터,바디스크럽,핸드크림,남성스킨케어,셀프태닝,메이크업브러쉬,"
            # 거치대류 (KC 불필요 비전기 — 무선/배터리는 KC제외에서 자동 차단)
            "스마트폰거치대,차량거치대,폰거치대,태블릿거치대,"
            # 반려동물 확장 (애견의류·캣타워·목욕용품)
            "애견의류,강아지옷,캣타워,펫침대,애견목욕,"
            # 홈데코 확장 (비전기 방향제·액자·드라이플라워)
            "아로마디퓨저,캔들홀더,벽걸이액자,패브릭포스터,드라이플라워,"
            # 스포츠 확장 (비전기 운동도구)
            "등산스틱,트레킹폴,수영킥판,스포츠타월,헬스밴드"
        )
    ).split(",") if kw.strip()
]
...
# 소싱 안전 카테고리 캐노니컬 분류 (2026-06-23 확정 — 대표님 소싱 규칙).
# 11개 그룹 × 세부 카테고리. KC 인증 불필요 군만 포함(식품·전자·완구안전인증·물놀이안전용품 제외).
# ⚠️ 스마트스토어는 추가로 _KC_EXCLUDE_KEYWORDS(유아/완구/전자/가스/식품 등)로 보수적 2차 필터링됨.
_SAFE_CATEGORIES: dict[str, list[str]] = {
    "패션의류":   ["여성의류", "남성의류", "아우터", "원피스", "티셔츠", "후드티", "맨투맨",
                   "바지", "반바지", "레깅스", "수영복", "래쉬가드", "이너웨어", "속옷"],
    "패션잡화":   ["가방", "모자", "양말", "신발", "슈즈", "스니커즈", "슬리퍼", "샌들",
                   "귀걸이", "목걸이", "반지", "팔찌", "시계", "선글라스", "벨트"],
    "뷰티":       ["스킨케어", "세럼", "앰플", "마스크팩", "헤어케어", "네일", "향수", "다이어트"],
    "생활/인테리어": ["청소", "수납정리", "욕실용품", "침구", "커튼", "러그", "조명", "무드등",
                   "인테리어", "원예", "가드닝", "공구"],
    "주방":       ["조리도구", "식기", "냄비", "프라이팬", "도마", "보온", "보냉"],
    "스포츠/아웃도어": ["캠핑", "텐트", "그늘막", "등산", "골프", "요가", "홈트레이닝",
                   "자전거", "낚시", "러닝", "여행용품"],
    "자동차":     ["차량용품", "방향제", "세차용품", "핸들커버"],
    "유아동/문구": ["유아의류", "장난감", "교육완구", "문구", "노트", "다이어리"],
    "여행":       ["캐리어", "여행파우치", "목베개", "트래블키트"],
    "펫용품":     ["반려동물"],
    "휴대폰액세서리": ["케이스", "충전기", "거치대", "이어폰", "보조배터리"],
}
# 소싱 안전 카테고리 — 대분류 게이트 (대표님 확정 21개 중 스마트스토어는 유아동/완구 제외 → 20개)
_SOURCING_GATE_CATEGORIES: tuple[str, ...] = (
    "패션잡화", "생활잡화", "주방용품", "휴대폰액세서리", "캠핑", "뷰티", "펫용품", "스포츠",
    "여성의류", "남성의류", "아우터", "이너웨어/속옷", "신발/슈즈", "주얼리/액세서리", "시계",
    "문구/취미/DIY", "자동차용품", "여행/캐리어", "인테리어/홈데코", "원예/가드닝",
    # ⚠️ "유아동/완구"는 어린이제품 KC 안전인증 필요 → 스마트스토어 소싱 제외 (카페24·쇼피파이만 허용)
)
...
_KC_EXCLUDE_KEYWORDS: tuple[str, ...] = (
    ...
    # 물놀이 안전용품(튜브·구명조끼 등 — 안전인증 대상)
    # ※ 수영복·래쉬가드(의류)는 SAFE이므로 "수영" 단독 제외하지 않음 (튜브/구명조끼만 차단)
    "물놀이", "튜브", "구명조끼", "구명", "스노클", "스노쿨", "수경", "물안경",
    # 어린이·유아용품
    "어린이", "유아", "아동", "키즈", "완구", "장난감", "유아용",
    # 가스용품
    "가스", "부탄", "토치", "버너",
    # 식품류(위생허가/식품표시 필요 — 2026-06-23 추가)
    "식품", "건강식품", "건강기능", "건강기능식품", "영양제", "비타민", "다이어트식품",
    "홍삼", "프로틴", "유산균", "효소",
    # 식품/건강기능식품 보강 (2026-06-25 추가)
    "도라지", "녹용", "녹각", "프로폴리스", "루테인", "오메가3", "콘드로이친", "쏘팔메토",
    "글루코사민", "멜라토닌", "밀크씨슬", "맥주효모", "비오틴", "콜라겐", "홍합", "마누카",
    "스피루리나", "흑마늘", "미숫가루", "올리브유", "팥앙금", "견과", "아몬드", "호두",
    "멸치", "다시마", "미역", "조미김", "재래김", "돌김", "김자반", "구운김",
    "소금", "천일염", "핑크소금", "설탕", "간장", "된장", "고추장", "쌈장",
    "조미료", "다시팩", "라면", "농심", "누룽지", "현미", "두유",
    # 의약외품
    "의약외품", "손소독", "구강청결", "밴드", "파스",
    # 안전인증 완구
    "교육완구", "안전인증완구",
    # 전선/케이블 (KC 전기용품 안전기준 적용 대상)
    "케이블",
)
...
CATEGORY_ID_MAP = {
    # ⚠️ 전부 네이버 실제 leaf 카테고리 ID로 교정 (2026-06-19). 기존 맵은 대부분 잘못된 ID였음
    #   (반려동물=50000430 트리없음, 뷰티=50000140 수영복, 주방=50002717 블루레이드라마 등)
    # 의류 (상의)
    "남성의류": 50000830, "여성의류": 50000803,
    "티셔츠": 50000830,  "후드티": 50000830,  "맨투맨": 50000830, "셔츠": 50000830,
    "니트": 50000830,    "조끼": 50000830,    "정장": 50000830,   "수트": 50000830,
    "블라우스": 50000803, "탑": 50000803,      "원피스": 50000803, "투피스": 50000803,
    "스커트": 50000803,   "20대패션": 50000803, "30대패션": 50000803,
    # 의류 (하의)
    "바지": 50000836,    "슬랙스": 50000836,  "반바지": 50000836, "트레이닝바지": 50000836,
    "청바지": 50000835,  "레깅스": 50000812,
    # 의류 (아우터)
    "아우터": 50000837,  "자켓": 50000837,    "코트": 50000837,   "패딩": 50000837,
    "바람막이": 50000837, "가디건": 50000837,  "점퍼": 50000837,   "트렌치코트": 50000837,
    # 패션잡화
    "패션잡화": 50003834,
    # 주방/생활/인테리어
    "주방": 50000216,    "주방용품": 50000216, "생활용품": 50002480,
    "청소": 50002476,    "수납정리": 50002464, "인테리어": 50003330,
    "침구": 50000969,    "욕실용품": 50002269, "원예": 50001782,   "공구": 50003405,
    # 뷰티
    "뷰티": 50000440,    "화장품": 50000440,  "헤어케어": 50000297,
    "네일": 50000402,    "향수": 50000312,    "다이어트": 50001899,
    # 식품/건강
    "식품": 50001899,    "건강": 50001899,
    # 스포츠/아웃도어
    "스포츠": 50002649,  "캠핑": 50002649,    "등산": 50002649,
    "골프": 50017520,    "자전거": 50002821,  "요가": 50017520,   "홈트레이닝": 50017520,
    "여행용품": 50007808, "자동차용품": 50003970,
    # 유아/완구/문구/도서
    "유아용품": 50000626, "완구": 50004210,   "문구": 50003563,   "도서": 50005754,
    # 반려동물
    "반려동물": 50000233,
    # 디지털/가전
    ...
    "구두": 50003834,    "부츠": 50003834,     "운동화": 50003834, "슈즈": 50003834,
    # 주얼리/액세서리/시계 → 패션잡화 leaf
    "주얼리": 50003834,  "귀걸이": 50003834,   "목걸이": 50003834, "반지": 50003834,
    "팔찌": 50003834,    "브로치": 50003834,   "액세서리": 50003834, "시계": 50003834,
    # 이너웨어/속옷 → 여성의류 leaf
    "이너웨어": 50000803, "속옷": 50000803,    "브라": 50000803,   "팬티": 50000803,
    "내복": 50000803,    "수면양말": 50000803,
    # 수영복/래쉬가드(의류 — SAFE, KC 불필요) → 여성의류 leaf
    "수영복": 50000803,  "래쉬가드": 50000803,
    # 식품/건강식품 → 다이어트/식품 leaf
    "간식": 50001899,    "과자": 50001899,     "음료": 50001899,   "건강식품": 50001899,
    "영양제": 50001899,  "다이어트식품": 50001899,
    # 유아동/완구 세분화
    ...
    "트래블키트": 50007808,
    # 인테리어/홈데코 세분화
    "벽지": 50003330,    "타일스티커": 50003330, "커튼": 50003330,  "블라인드": 50003330,
    "러그": 50003330,    "액자": 50003330,
    # 문구 세분화
    "노트": 50003563,    "펜": 50003563,        "다이어리": 50003563, "스티커": 50003563,
    "포스트잇": 50003563,
}
DEFAULT_CATEGORY_ID = 50002480  # 생활용품(갑티슈) — 유효 leaf 안전 기본값
...
_STYLE_KEYWORDS = {
    # C. 브랜드 감성형 — 프리미엄 뷰티/스킨케어/향수/주얼리/시계 (모호어 제외: 크림(색)/선블록(래쉬가드))
    "C": ["스킨케어", "세럼", "앰플", "에센스", "토너", "스킨로션", "바디로션", "향수", "퍼퓸",
          "아이크림", "핸드크림", "수분크림", "보습크림", "마스크팩", "화장품", "안티에이징", "미백",
          "시계", "주얼리", "프리미엄", "럭셔리"],
    # B. 인포그래픽형 — 캠핑/스포츠/아웃도어/헬스/자동차/건강기능
    "B": ["캠핑", "텐트", "그늘막", "등산", "아웃도어", "스포츠", "헬스", "요가", "골프",
          "자전거", "낚시", "러닝", "트레킹", "웨이트", "수영용품", "물놀이용품", "스쿠버", "서핑",
          "자동차", "블랙박스", "세차", "차량용품", "영양제", "건강식품", "킥보드"],
    # D. 썸네일 임팩트형 — 생활잡화/주방/청소/수납/인테리어/식품/문구/완구
    "D": ["주방", "조리", "식기", "냄비", "프라이팬", "도마", "청소", "세제", "수납", "정리",
          "욕실", "인테리어", "조명", "무드등", "생활", "걸레", "행거", "선반", "옷걸이", "보온", "보냉",
          "식품", "간식", "다이어트", "원예", "가드닝", "문구", "완구", "커튼", "러그"],
    # A(기본) — 패션/의류/신발/주얼리(저가)/가방/펫/유아동/여행 (C·B·D 미매칭 시 fallback)
    "A": ["신발", "슬리퍼", "샌들", "구두", "부츠", "운동화", "귀걸이", "목걸이", "반지", "팔찌",
          "브로치", "이너웨어", "속옷", "브라", "내복", "여행", "캐리어", "여행파우치", "목베개",
          "가방", "지갑", "벨트", "모자", "양말", "스카프", "장갑", "반려동물", "강아지", "고양이",
          "유아의류", "아기용품"],
}

_STYLE_A_OVERRIDE = ("반려동물", "강아지", "고양이", "펫", "유아", "아기", "신생아", "키즈", "아동")

def _html_style_for(category: str, name: str = "") -> str:
    """카테고리·상품명 → HTML 스타일 키(A/B/C/D). 우선순위 C(프리미엄뷰티) > B(스포츠) > D(생활) > A(기본)."""
    t = f"{category} {name}"
    # 펫·유아동은 스토리텔링(A) 우선 — '급식기→식기' 등 부분일치 오매칭 방지
    if any(k in t for k in _STYLE_A_OVERRIDE):
        return "A"
    for st in ("C", "B", "D"):
...
# ─── Job 영속성: 재배포 후 재개 지원 (context_store REST API 기반) ──────────────
# Railway 크로스 프로젝트 간 psycopg2 직접연결 불가 → context_store HTTP API 사용
# (context_store 자체는 동일 PostgreSQL을 백엔드로 사용함)
_JOB_KEY = "ss_job.reapply_html"
_CTX_API = "https://loving-serenity-production-2635.up.railway.app"


async def jobs_table_create() -> None:
    pass  # context_store는 별도 테이블 생성 불필요


async def job_create(job_type: str, total: int = 0) -> int | None:
    """잡 생성 → context_store 저장 → 항상 id=1 반환 (단일 reapply 잡)."""
    try:
        _now = datetime.now(timezone.utc).isoformat()
        _job = {"id": 1, "type": job_type, "status": "running", "total": total,
                "processed": 0, "last_processed_id": "", "error": "",
                "started_at": _now, "updated_at": _now}
        async with httpx.AsyncClient(timeout=8) as _c:
            await _c.post(f"{_CTX_API}/context",
                          json={"key": _JOB_KEY, "value": json.dumps(_job), "category": "ss_jobs"})
        return 1
...
async def job_update(job_id: int | None, processed: int, last_id: str = "",
                     error: str = "", total: int | None = None) -> None:
    if not job_id:
        return
    try:
        async with httpx.AsyncClient(timeout=8) as _c:
            _r = await _c.get(f"{_CTX_API}/context/{_JOB_KEY}")
            _raw = _r.json().get("value", "{}") if _r.status_code == 200 else "{}"
            _job = json.loads(_raw) if isinstance(_raw, str) else {}
            _job.update({"processed": processed, "last_processed_id": last_id,
...
async def job_finish(job_id: int | None, processed: int, error: str = "") -> None:
    if not job_id:
        return
    try:
        ...
        async with httpx.AsyncClient(timeout=8) as _c:
            ...
            _raw = _r.json().get("value", "{}") if _r.status_code == 200 else "{}"
            _job = json.loads(_raw) if isinstance(_raw, str) else {}
            _job.update({"status": _st, "processed": processed, "error": error,
                         "updated_at": datetime.now(timezone.utc).isoformat()})
            await _c.post(f"{_CTX_API}/context",
                          json={"key": _JOB_KEY, "value": json.dumps(_job), "category": "ss_jobs"})
    except Exception as _je:
...
async def job_get(job_id: int = 1) -> dict | None:
    try:
        async with httpx.AsyncClient(timeout=8) as _c:
            _r = await _c.get(f"{_CTX_API}/context/{_JOB_KEY}")
        if _r.status_code != 200:
        ...
        return json.loads(_raw) if isinstance(_raw, str) else (_raw or None)
    except Exception as _je:
        print(f"[JOB] job_get 실패: {_je}", flush=True)
        return None


async def job_get_running(job_type: str = "reapply-html") -> dict | None:
    try:
        async with httpx.AsyncClient(timeout=8) as _c:
            _r = await _c.get(f"{_CTX_API}/context/{_JOB_KEY}")
        if _r.status_code != 200:
            return None
        _raw = _r.json().get("value", "{}")
        _job = json.loads(_raw) if isinstance(_raw, str) else None
        if _job and _job.get("type") == job_type and _job.get("status") == "running":
            return _job
        return None
    except Exception as _je:
...
async def pipeline_reapply_claude_html(limit: int = 0, nos: list | None = None, job_id: int | None = None) -> dict:
    ...
    if job_id:
        await job_update(job_id, _proc_offset, "", "", total=_proc_offset + len(not_applied))
    await _tg_notify(
    ...
    for idx, item in enumerate(not_applied, 1):
        ...
        if job_id:
            await job_update(job_id, _proc_offset + idx, product_id)
        await asyncio.sleep(2.5)

    # 잡 완료 기록
    if job_id:
        await job_finish(job_id, _proc_offset + results["success"] + results["failed"],
...
_TG_NOISE_PATTERNS = (
    ...
    # 최소화 (2026-06-20): 등록완료/소싱/집계·리포트 차단 (주문/오류는 통과)
    "[스마트스토어 등록완료]", "등록완료", "등록 완료", "소싱 완료", "소싱완료",
    "주간 리포트", "주간리포트", "일일 리포트", "일일리포트", "매출 요약", "매출집계", "헬스체크",
)
def _tg_is_noise(msg: str) -> bool:
    """진행·소싱·등록완료·집계 알림 차단. 주문/오류만 통과 (2026-06-20 최소화)."""
    if not msg:
        return False
